...
import { Users, Trophy, BookOpen, Clock, RefreshCw, TrendingUp, Shield } from 'lucide-react'
...
export default function Dashboard() {
  ...
  const fmtDate = (iso) => new Date(iso).toLocaleDateString('ko-KR', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard icon={TrendingUp} label="평균 완료"     value={stats.totalPlayers > 0 ? (stats.totalCompletions / stats.totalPlayers).toFixed(1) : '0'} color="purple" />
      </div>
      ...
      <div className="flex gap-2 border-b border-gray-700 pb-1">
        {[['overview','전체 현황'], ['teams','팀별'], ['players','사용자별']].map(([k, l]) => (
          <button key={k} onClick={() => setTab(k)}
            className={`px-4 py-1.5 rounded-t-lg text-sm font-medium transition ${tab === k ? 'bg-gray-700 text-white' : 'text-gray-400 hover:text-white'}`}>
            {l}
          </button>
        ))}
      </div>
      ...
      {tab === 'overview' && (
        <div className="space-y-6">
          ...
          <div className="bg-gray-800 rounded-xl border border-gray-700 p-5">
            ...
            {stats.recent.length === 0 ? <p className="text-gray-500 text-sm">기록 없음</p> : (
                ...
                {stats.recent.map((r) => (
                  <div key={r.id} className="flex items-center gap-3 py-2 border-b border-gray-700 last:border-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-white truncate">{r.player_name || r.nickname}
      ...
      {tab === 'players' && (
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-5">
          <h3 className="text-base font-semibold text-white mb-4">
            사용자 목록 <span className="text-gray-500 text-sm font-normal">({stats.players.length}명)</span>
          </h3>
          {stats.players.length === 0 ? (
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {stats.players.map((p, i) => (
                <div key={`${p.name}_${p.teamId}`} className="flex items-center gap-3 py-2 border-b border-gray-700 last:border-0">
                  <span className="text-xs text-gray-500 w-5 text-right">{i + 1}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-white font-medium truncate">{p.name}</p>
                    <p className="text-xs text-gray-500">{p.teamName} · {fmtDate(p.lastAt)}</p>
                  </div>
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 bg-green-500/10 text-green-400 rounded-full text-xs shrink-0">
