...
export default function TitlePage({ onStart, onContinue, canContinue, onTap, bgmStarted }) {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('biblequest_nickname')
    if (saved) { setNickname(saved); setHasNickname(true) }
  }, [])
  ...
  return (
    <div
      ...
      onClick={!bgmStarted ? onTap : undefined}
    >
      {/* 배경 */}
      ...
      <div className="relative z-10 size-full flex flex-col items-center justify-between p-10 md:p-16">
        ...
        {/* 버튼 영역 */}
        <motion.div className="flex-none w-full max-w-md space-y-5"
